...
import { TalentLogo } from './components/ui/TalentLogo';
...
function MainLayout() {
  ...
  const modules: AppModule[] = [
    {
      ...
      color: 'bg-navy-50 text-accent border-border',
      minTier: 'freemium',
      version: 'v4.0'
    },
    {
    ...
    },
    {
      ...
      icon: <FileText size={20} />,
      color: 'bg-warning-bg text-warning-text border-warning-text/20',
      minTier: 'enterprise',
      version: 'v1.0'
    },
    {
    ...
    if (customizations) {
      ...
      return {
        ...
        icon: customizations.iconName && IconMap[customizations.iconName]
          ...
          : mod.icon,
        color: customColor ? customColor.sidebar : moduleTheme.sidebar
      };
    }
    return { ...mod, color: moduleTheme.sidebar };
  });
  ...
  return (
    <div className="flex h-[100dvh] bg-page font-sans overflow-hidden selection:bg-accent-soft relative">
      ...
      {/* MAIN CONTENT AREA */}
      <main className="flex-1 flex flex-col min-w-0 bg-page relative z-0">
        {/* Top Navbar */}
          ...
          <div className="h-20 flex items-center justify-between px-4 lg:px-8">
            ...
            <div className="flex items-center gap-2 sm:gap-3.5 min-w-0">
              ...
              <button
                ...
                className="lg:hidden p-2 -ml-2 rounded-xl text-text-3 hover:bg-page hover:text-text-2 transition-colors focus:outline-none shrink-0"
            ...
            <div className="flex items-center gap-2 sm:gap-4 shrink-0">
              ...
              <div className="relative" ref={profileMenuRef}>
                <button
                  ...
                  className="flex items-center gap-2 sm:gap-3 hover:bg-page p-1 sm:p-1.5 rounded-2xl transition-colors focus:outline-none select-none text-left"
                >
                  ...
                  <div className="absolute right-0 mt-2 w-52 sm:w-56 bg-white border border-border rounded-2xl shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="p-1.5 space-y-0.5">
                      <button
                        ...
                        className="w-full text-left px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold text-text-2 hover:bg-page transition-colors flex items-center gap-2"
                      >
                      ...
                      <EnlaceAlAviso className="no-underline block">
                        <span className="w-full text-left px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold text-text-2 hover:bg-page transition-colors flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="w-full text-left px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold text-danger-text hover:bg-danger-bg transition-colors flex items-center gap-2"
                      >
  ...
  if (banReason) {
    return (
      <div className="w-screen h-[100dvh] bg-slate-950 flex flex-col items-center justify-center p-6 text-center select-none font-sans relative overflow-hidden">
        {/* Glow Effects */}
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] bg-danger-icon/10 rounded-full blur-[100px] pointer-events-none"></div>

        <div className="bg-slate-900/40 border border-danger-text/20 backdrop-blur-xl p-8 rounded-3xl max-w-md w-full shadow-2xl relative z-10 animate-in zoom-in-95">
          <div className="w-16 h-16 bg-danger-icon/10 border border-danger-text/30 rounded-2xl flex items-center justify-center mx-auto mb-6 text-danger-text shadow-inner">
            <Lock size={32} className="animate-pulse" />
          </div>

          <h1 className="text-2xl font-black text-white tracking-tight leading-none mb-3">
            Acceso <span className="text-danger-text">Suspendido</span>
          </h1>
          ...
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-danger-text to-danger-text hover:from-danger-icon hover:to-danger-text text-white font-bold text-xs uppercase tracking-widest rounded-xl shadow-lg shadow-danger-text/20 transition-all duration-300 transform active:scale-95"
          >
            Reintentar Conexión
          </button>
        </div>
    ...
    <Routes>
      ...
      <Route path="/superadmin" element={
        <ProtectedRoute allowedRoles={['platform_admin']}>
          <div className="min-h-[100dvh] bg-page p-3 sm:p-5 md:p-8 overflow-y-auto">
            <SaaSPlatformAdmin />
          </div>
        </ProtectedRoute>
      } />
      ...
      <Route path="/empleado" element={
        <ProtectedRoute allowedRoles={['empleado']}>
          <div className="w-screen h-[100dvh] bg-page overflow-hidden flex m-0 p-0 select-none">
