...
export default function GestorVacantes() {
  ...
  if (loading) return <div className="p-8 text-center text-text-3">Cargando bolsa de trabajo...</div>;

  return (
    <div className="bg-white rounded-3xl p-4 sm:p-6 border border-border relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h2 className="text-xl font-bold text-text-1 flex items-center gap-2">
            <Briefcase size={20} className="text-accent" /> Gestor de Vacantes (ATS)
          </h2>
          <p className="text-sm text-text-3 mt-1">Controla las vacantes visibles en la Web Pública de Empleos.</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <button
            onClick={() => setShowQRModal(true)}
            className="w-full sm:w-auto justify-center bg-white border border-border text-text-2 hover:bg-page px-4 py-2.5 rounded-lg font-medium shadow-sm transition-all text-sm flex items-center gap-2"
          >
            <QrCode size={16} /> Ver QR Público
          </button>
          <button
            onClick={openNewModal}
            className="w-full sm:w-auto justify-center bg-accent hover:bg-accent-hover text-white px-5 py-2.5 rounded-lg font-medium shadow-sm transition-all text-sm flex items-center gap-2"
          >
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-page border-b border-border text-text-3 text-xs uppercase tracking-wider">
          ...
          <tbody className="divide-y divide-border">
            {vacancies.map(v => (
              <tr key={v.id} className="hover:bg-page transition-colors">
                <td className="p-4">
                  <span className="inline-block bg-page text-text-2 px-2.5 py-1 rounded-md text-xs font-bold border border-border">
                    {v.role_name}
                  </span>
                </td>
                <td className="p-4">
                  <p className="font-bold text-text-1 text-sm">{v.title}</p>
                </td>
                <td className="p-4 text-sm text-text-2">{v.work_type}</td>
                <td className="p-4 text-sm text-success-text font-semibold">{v.salary_range}</td>
                <td className="p-4 text-center">
                  <button
                    onClick={() => toggleStatus(v.id, v.is_active)}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${v.is_active ? 'bg-accent' : 'bg-slate-300'}`}
                  >
                ...
                <td className="p-4 text-sm whitespace-nowrap">
                  <button
                    ...
                    className="text-slate-400 hover:text-accent font-medium p-2 hover:bg-navy-50 rounded-lg transition-colors"
                  >
                    <Edit2 size={16} />
                  </button>
                  <button
                    ...
                    className="text-slate-400 hover:text-danger-text font-medium p-2 hover:bg-danger-bg rounded-lg transition-colors"
                  >
                    <Trash2 size={16} />
                  </button>
                </td>
          ...
          <div key={v.id} className="bg-white border border-border rounded-2xl p-4 shadow-sm relative">
            <div className="flex justify-between items-start gap-4 mb-3">
              ...
              <div className="flex shrink-0">
                <button
                  onClick={() => openEditModal(v)}
                  className="text-slate-400 hover:text-accent p-1.5 hover:bg-navy-50 rounded-lg transition-colors"
                >
                  <Edit2 size={16} />
                </button>
                <button
                  onClick={() => eliminarVacante(v)}
                  className="text-slate-400 hover:text-danger-text p-1.5 hover:bg-danger-bg rounded-lg transition-colors"
                >
                  <Trash2 size={16} />
                </button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs border-t border-border pt-3">
              <div>
                ...
                <p className="font-semibold text-text-2">{v.work_type}</p>
              </div>
              <div>
                <p className="text-slate-400">Salario</p>
                <p className="font-semibold text-success-text">{v.salary_range}</p>
              </div>
              <div className="col-span-2 flex items-center justify-between mt-1 bg-page p-2 rounded-xl">
                <span className="text-xs text-text-3 font-medium">Publicada en Web Pública</span>
                <button
                  onClick={() => toggleStatus(v.id, v.is_active)}
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none shrink-0 ${v.is_active ? 'bg-accent' : 'bg-slate-300'}`}
                >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-4xl w-full p-4 sm:p-8 shadow-2xl border border-border relative my-8 animate-slide-up max-h-[90vh] overflow-y-auto">
             <button onClick={() => setShowModal(false)} className="absolute top-6 right-6 text-slate-400 hover:text-text-2 bg-page p-2 rounded-full transition-all"><X size={20}/></button>
            <h3 className="text-2xl font-black text-text-1 mb-6 flex items-center gap-2">
            ...
            <form onSubmit={handleSave} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <div className="bg-page p-5 rounded-2xl border border-border space-y-4">
                    <h4 className="font-bold text-xs uppercase tracking-wider text-slate-400 mb-2">Información de la Vacante</h4>

                    <div>
                      <label className="block text-xs font-bold text-text-2 uppercase mb-1.5 ml-1">Título Público de la Vacante</label>
                      <input
                        type="text"
                        required
                        className="w-full bg-white border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus-visible:ring-focus-ring/20 focus:border-accent transition-all font-semibold"
                    ...
                    <div>
                      <label className="block text-xs font-bold text-text-2 uppercase mb-1.5 ml-1">Puesto Oficial (Backend)</label>
                      <select
                        className="w-full bg-white border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus-visible:ring-focus-ring/20 focus:border-accent transition-all font-semibold"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-text-2 uppercase mb-1.5 ml-1">Modalidad</label>
                        <select
                          className="w-full bg-white border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus-visible:ring-focus-ring/20 focus:border-accent transition-all font-semibold"
                      ...
                      <div>
                        <label className="block text-xs font-bold text-text-2 uppercase mb-1.5 ml-1">Rango Salarial</label>
                        <input
                          type="text"
                          className="w-full bg-white border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus-visible:ring-focus-ring/20 focus:border-accent transition-all font-semibold"
                    ...
                    <div>
                      <label className="block text-xs font-bold text-text-2 uppercase mb-1.5 ml-1">Horario y Jornada</label>
                      <input
                        type="text"
                        className="w-full bg-white border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus-visible:ring-focus-ring/20 focus:border-accent transition-all font-semibold"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-text-2 uppercase mb-1.5 ml-1">Descripción de la vacante</label>
                    <textarea
                      className="w-full bg-page border border-border rounded-xl px-4 py-3 text-sm h-32 focus:outline-none focus:ring-2 focus-visible:ring-focus-ring/20 focus:border-accent transition-all font-semibold leading-relaxed"
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-page p-5 rounded-2xl border border-border space-y-4 flex flex-col h-[280px]">
                    ...
                    <div className="flex gap-2">
                      <input
                        type="text"
                        className="flex-grow bg-white border border-border rounded-xl px-4 py-2 text-xs focus:outline-none focus:ring-2 focus-visible:ring-focus-ring/20 focus:border-accent font-semibold"
                      ...
                      <button
                        ...
                        className="bg-accent hover:bg-accent-hover text-white font-bold text-xs px-4 py-2 rounded-xl transition-all shadow-sm shrink-0"
                      >
                        Agregar
                      </button>
                    </div>

                    <div className="flex-grow overflow-y-auto bg-white border border-border rounded-xl p-3 space-y-2 custom-scrollbar">
                      {formData.requirements.length === 0 ? (
                        ...
                        formData.requirements.map((req, idx) => (
                          <div key={idx} className="flex justify-between items-center gap-2 bg-page border border-border p-2 rounded-lg text-xs font-semibold text-text-2 hover:bg-page transition-colors">
                            <span className="truncate">{req}</span>
                            <button
                              type="button"
                              onClick={() => handleRemoveRequirement(idx)}
                              className="text-slate-400 hover:text-danger-text p-1 rounded transition-colors"
                  ...
                  <div className="bg-page p-5 rounded-2xl border border-border space-y-4">
                    ...
                    <div className="h-24 w-full bg-slate-200 rounded-xl overflow-hidden relative border border-border">
                      {formData.image_url ? (
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {PRESET_IMAGES.map((img, idx) => (
                          <button
                            ...
                            className={`group relative h-12 rounded-lg overflow-hidden border transition-all ${formData.image_url === img.url ? 'ring-2 ring-focus-ring border-accent' : 'border-border hover:border-navy-300'}`}
                            title={img.name}
                          >
              ...
              <div className="flex gap-4 pt-4 mt-6 border-t border-border">
                <button
                  ...
                  className="flex-1 bg-page text-text-2 hover:bg-slate-200 px-4 py-3.5 rounded-xl font-bold transition-all text-sm"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="flex-1 bg-accent hover:bg-accent-hover text-white px-4 py-3.5 rounded-xl font-bold shadow-md transition-all text-sm shadow-accent/10"
                >
                  Guardar Vacante
                </button>
              </div>
      ...
      {showQRModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white rounded-3xl max-w-sm w-full p-8 shadow-xl border border-border relative overflow-hidden flex flex-col items-center text-center">
             <button onClick={() => setShowQRModal(false)} className="absolute top-6 right-6 text-slate-400 hover:text-text-2 bg-page p-2 rounded-full"><X size={20}/></button>
             <QrCode size={48} className="text-accent mb-4" />
             <h3 className="text-2xl font-bold text-text-1 mb-2">Portal Público</h3>
             <p className="text-sm text-text-3 mb-4">Imprime este código QR y colócalo en la entrada de la sucursal o compártelo en redes sociales.</p>
             ...
             {portalSlug ? (
               ...
               <div className="bg-warning-bg border border-warning-text/20 rounded-2xl p-4 w-full text-left mb-4">
                 <p className="text-xs text-warning-text font-semibold">Todavía no se pudo leer la dirección pública de tu empresa. Ábrela en la pestaña «Vista Pública (Web)» y vuelve a intentarlo.</p>
               </div>
             )}
             ...
             {isLocalhost() && (
                <div className="p-3 bg-warning-bg border border-warning-text/80 rounded-xl text-left w-full mb-4">
                  ...
                  <input
                    ...
                    className="w-full text-[11px] bg-white border border-warning-text/20 px-2 py-1 rounded-md text-text-2 focus:outline-none focus:ring-1 focus-visible:ring-warning-text placeholder-slate-400 font-mono"
                  />
                </div>
             )}

             <p className="text-xs font-bold text-accent bg-navy-50 px-3 py-1.5 rounded-lg border border-border">¡Escanea para probar!</p>
          </div>
        </div>
      )}
    </div>
