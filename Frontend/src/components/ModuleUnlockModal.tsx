...
export const ModuleUnlockModal: React.FC<ModuleUnlockModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl border border-border w-full max-w-xl overflow-hidden relative flex flex-col max-h-[90vh]">

        {/* Header */}
        <div className="p-6 bg-gradient-to-r from-slate-900 via-brand-dark to-slate-900 text-white relative">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-4">
            <div className="p-3 bg-accent/20 text-navy-300 rounded-2xl border border-navy-300/30 shrink-0">
        ...
        <div className="p-6 overflow-y-auto space-y-5">
          {successMessage && (
            <div className="p-4 bg-success-bg border border-success-text/20 rounded-2xl flex items-center gap-3 text-success-text text-xs font-bold animate-in fade-in">
          ...
          {errorMessage && (
            <div className="p-4 bg-danger-bg border border-danger-text/20 rounded-2xl text-danger-text text-xs font-bold animate-in fade-in">
              {errorMessage}
            </div>
          )}
          ...
          {activeTab === 'social' ? (
            <form onSubmit={handleSubmitSocialGrace} className="space-y-4">
              <div className="p-4 bg-navy-50/60 rounded-2xl border border-border space-y-2">
                <div className="flex items-center gap-2 text-brand-dark font-extrabold text-xs">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <button
                    ...
                    className="p-2.5 rounded-xl border border-border hover:border-accent hover:bg-navy-50 text-text-2 text-xs font-bold flex items-center justify-center gap-2 transition-all"
                  >
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-xl border border-border hover:border-success-text hover:bg-success-bg text-text-2 text-xs font-bold flex items-center justify-center gap-2 transition-all"
                  >
                    <MessageCircle size={16} className="text-success-text" />
                    WhatsApp
                  </button>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-text-2 mb-1">
                ...
                <input
                  ...
                  className="w-full text-xs p-3 rounded-xl border border-border focus:outline-none focus:ring-2 focus-visible:ring-focus-ring/20 focus:border-accent"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 bg-accent hover:bg-accent-hover text-white font-extrabold text-xs rounded-xl shadow-md transition-all flex items-center justify-center gap-2"
              >
            ...
            <div className="space-y-5">
              <div className="p-5 rounded-2xl bg-page border border-border space-y-3">
                <div className="flex justify-between items-baseline">
                  <span className="text-xs font-bold text-text-3 uppercase tracking-wider">Tarifa A la Carta</span>
                  <div className="text-right">
                    <span className="text-2xl font-black text-text-1">${moduleData.pricePerColab || 15} MXN</span>
                    <span className="text-[11px] text-text-3 font-bold"> / colaborador / mes</span>
                  </div>
                </div>
              ...
              <button
                ...
                disabled={isSubmitting}
                className="w-full py-3.5 bg-gradient-to-r from-accent to-accent hover:from-accent-hover hover:to-accent-hover text-white font-extrabold text-xs rounded-xl shadow-lg transition-all flex items-center justify-center gap-2"
              >
                {isSubmitting ? 'Contratando...' : 'Contratar Módulo A la Carta'}
                <Zap size={16} />
              </button>

              <div className="p-4 bg-warning-bg border border-warning-text/20 rounded-2xl flex items-center justify-between gap-3">
                <div>
                  <h4 className="text-xs font-extrabold text-warning-text">¿Prefieres acceso total a todo la suite?</h4>
                  <p className="text-[11px] text-warning-text mt-0.5">Actualiza al Plan PRO o Enterprise desde $29 MXN/colaborador.</p>
                </div>
                <button
                  onClick={() => {
                    onClose();
                  ...
                  className="px-3.5 py-2 bg-warning-text hover:bg-warning-text text-white text-xs font-extrabold rounded-xl shrink-0 transition-colors"
                >
                  Ver Planes
                </button>
              </div>
