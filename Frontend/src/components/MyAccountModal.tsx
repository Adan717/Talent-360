...
export const MyAccountModal = ({ isOpen, onClose }: MyAccountModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 selection:bg-accent-soft animate-in fade-in duration-200">
      ...
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-border overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200">

        {/* Header */}
        <div className="bg-slate-900 text-white p-6 relative overflow-hidden shrink-0">
          <div className="absolute top-0 left-0 w-full h-full bg-accent/10 blur-3xl"></div>
        ...
        {/* Tabs Bar */}
        <div className="flex border-b border-border bg-page/50 p-2 shrink-0 overflow-x-auto custom-scrollbar">
          <button
            onClick={() => setActiveTab('profile')}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-extrabold text-xs transition-all ${activeTab === 'profile' ? 'bg-white text-accent shadow-sm border border-border' : 'text-text-3 hover:text-text-2'}`}
          >
            <User size={14} />
            Mi Perfil
          </button>
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-extrabold text-xs transition-all ${activeTab === 'security' ? 'bg-white text-accent shadow-sm border border-border' : 'text-text-3 hover:text-text-2'}`}
          >
            <Key size={14} />
            Seguridad y Accesos
          </button>
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-extrabold text-xs transition-all ${activeTab === 'rest_day' ? 'bg-white text-accent shadow-sm border border-border' : 'text-text-3 hover:text-text-2'}`}
          >
            <Calendar size={14} />
            Horario y Descanso
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 min-h-0 custom-scrollbar">
          ...
          {activeTab === 'profile' && (
            <form onSubmit={handleUpdateProfile} className="space-y-6">
              ...
              <div className="bg-page p-4 rounded-2xl border border-border flex items-center gap-4">
                <div
              ...
              {/* Edit Name */}
              <div>
                <label className="block text-xs font-bold text-text-2 uppercase tracking-wider mb-2">Nombre Completo</label>
                <input
                  type="text"
                  required
                  value={name}
                  onChange={e => setName(e.target.value)}
                  className="w-full px-4 py-2.5 bg-page border border-border rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus-visible:ring-focus-ring focus:border-transparent transition-all font-medium text-text-1 text-sm"
              ...
              <div>
                <label className="block text-xs font-bold text-text-2 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <Sparkles size={12} className="text-warning-text" />
                  Selecciona una Imagen de Perfil
                </label>
                <div className="grid grid-cols-4 sm:grid-cols-8 gap-2 bg-page p-3 rounded-2xl border border-border">
                  {PRESET_AVATARS.map((urlOption, index) => (
                    <button
                      key={index}
                      type="button"
                      onClick={() => setAvatar(urlOption)}
                      className={`w-12 h-12 rounded-full border-2 overflow-hidden transition-all relative ${avatar === urlOption ? 'border-accent scale-105 shadow-sm shadow-accent/20' : 'border-transparent hover:border-slate-300'}`}
                    >
              ...
              {/* Custom Avatar URL */}
              <div>
                <label className="block text-xs font-bold text-text-2 uppercase tracking-wider mb-2">O pega una URL de Imagen personalizada</label>
                <input
                  type="url"
                  value={avatar}
                  onChange={e => setAvatar(e.target.value)}
                  className="w-full px-4 py-2.5 bg-page border border-border rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus-visible:ring-focus-ring focus:border-transparent transition-all font-medium text-text-1 text-xs"
              ...
              <div className="bg-page p-4 rounded-2xl border border-border">
                <label className={`flex items-center justify-between gap-3 ${isNewHireForAcademyToggle ? 'opacity-60' : 'cursor-pointer'}`}>
                  ...
                  <input
                    ...
                    className="w-5 h-5 text-accent rounded border-slate-350 shrink-0"
                  />
                </label>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="bg-accent hover:bg-accent-hover text-white font-extrabold text-xs px-5 py-2.5 rounded-xl transition-all shadow-md shadow-accent/10 flex items-center gap-1.5 disabled:opacity-75"
                >
          ...
          {activeTab === 'security' && (
            <form onSubmit={handleChangePassword} className="space-y-5">
              ...
              {passwordError && (
                <div className="bg-danger-bg border border-danger-text/20 text-danger-text text-xs font-bold p-3.5 rounded-xl flex items-start gap-2 animate-in fade-in duration-200">
              ...
              {passwordSuccess && (
                <div className="bg-success-bg border border-success-text/20 text-success-text text-xs font-bold p-3.5 rounded-xl flex items-start gap-2 animate-in fade-in duration-200">
              ...
              <div>
                <label className="block text-xs font-bold text-text-2 uppercase tracking-wider mb-2">Contraseña Actual</label>
                <input
                  type="password"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-text-2 uppercase tracking-wider mb-2">Nueva Contraseña</label>
                <input
                  type="password"
                  required
                  value={newPassword}
              ...
              <div>
                <label className="block text-xs font-bold text-text-2 uppercase tracking-wider mb-2">Confirmar Nueva Contraseña</label>
                <input
                  type="password"
                  required
          ...
          {activeTab === 'rest_day' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-page p-4 rounded-2xl border border-border flex items-center gap-3">
                  <div className="p-2.5 bg-navy-50 text-accent rounded-xl shrink-0">
                    <Clock size={18} />
                  </div>
                  <div>
                ...
                <div className="bg-page p-4 rounded-2xl border border-border flex items-center gap-3">
                  <div className="p-2.5 bg-success-bg text-success-text rounded-xl shrink-0">
                    <Calendar size={18} />
                  </div>
                  <div>
              ...
              <form onSubmit={handleRequestRestDay} className="bg-page/50 p-5 rounded-2xl border border-border space-y-4">
                <h4 className="font-extrabold text-text-1 text-sm flex items-center gap-1.5">
                  <Send size={14} className="text-accent" />
                  Solicitar Cambio de Día de Descanso
                </h4>

                {restDayError && (
                  <div className="bg-danger-bg border border-danger-text/20 text-danger-text text-xs font-bold p-3 rounded-xl flex items-start gap-2">
                ...
                {restDaySuccess && (
                  <div className="bg-success-bg border border-success-text/20 text-success-text text-xs font-bold p-3 rounded-xl flex items-start gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 bg-white border border-border rounded-xl focus:outline-none focus:ring-2 focus-visible:ring-focus-ring focus:border-transparent font-semibold text-text-2 text-xs"
                    >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-white border border-border rounded-xl focus:outline-none focus:ring-2 focus-visible:ring-focus-ring focus:border-transparent font-medium text-text-2 text-xs resize-none"
                  ></textarea>
                </div>
              ...
              <div>
                <h4 className="font-extrabold text-text-1 text-xs uppercase tracking-wider mb-3">Historial de Solicitudes</h4>
                {restDayRequests.length === 0 ? (
                  <p className="text-slate-400 text-xs font-semibold text-center py-4 bg-page rounded-2xl border border-dashed border-border">
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      ...
                      <tbody className="divide-y divide-border">
                        {restDayRequests.map((req) => (
                          <tr key={req.id} className="hover:bg-page/50">
                            ...
                            <td className="p-3 font-medium text-text-2 min-w-[200px]">
