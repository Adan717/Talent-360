...
import { LabMobileBottomNav as MobileBottomNav } from './LabMobileBottomNav';
import { LabPhoneTasks } from './LabPhoneTasks';
...
export const LabClockSimulator: React.FC<RelojSimuladoLandingProps> = ({
  tier,
  setTier,
  onActionClick,
  empName = 'Colaborador de ejemplo',
  storeName = 'Empresa de ejemplo'
}) => {
  ...
  // Estados de fichajes simulados para una cuenta de ejemplo.
  ...
  // El tiempo de la demo vive únicamente dentro del teléfono.
  useEffect(() => {
    let timer: any;
  ...
  const handleSendMsg = () => {
    ...
    schedule(() => {
      ...
      if (userMessage.toLowerCase().includes('ayuda') || userMessage.toLowerCase().includes('soporte')) {
        ...
        botResponse = `Hecho. La tarea operativa se ha generado y asignado al tablero de ${empName}.`;
  ...
  return (
    <div className={`w-full h-full flex flex-col justify-between overflow-hidden relative ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-page text-text-1'}`}>
      ...
      {demoNotice && <div className="lab-phone-notice" role="status"><p>{demoNotice}</p><button type="button" onClick={() => setDemoNotice('')} aria-label="Cerrar aviso del simulador"><X size={16} /></button></div>}
          ...
          <div className="flex-1 flex flex-col justify-between py-2 text-left overflow-y-auto scrollbar-none">
            {tier === 'free' ? (
              ...
              <div className="flex-grow flex flex-col min-h-0">
                <LabPhoneTasks firstDone={simTask1Done} secondDone={simTask2Done} onFirstDone={setSimTask1Done} onSecondDone={setSimTask2Done} />
              </div>
            )}
          </div>
        )}
      ...
      <div style={{ transform: 'scale(0.85)', transformOrigin: 'bottom center' }} className="absolute bottom-1 inset-x-0 z-30 shrink-0 flex items-center justify-center px-2">
        <div className="w-full max-w-[420px] relative">
          <MobileBottomNav
            ...
            isDark={isDark}
            clockState={clockState}
            showCustomAlert={setDemoNotice}
          />
          <div className="absolute right-[2px] bottom-[2px] z-40">
            <button
              type="button"
              onClick={() => setIsFabSheetOpen(prev => !prev)}
              aria-label={isFabSheetOpen ? 'Cerrar herramientas del simulador' : 'Abrir herramientas del simulador'}
