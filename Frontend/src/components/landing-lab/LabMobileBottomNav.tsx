...
export function LabMobileBottomNav({
  ...
  const checadorColor = 'var(--lab-accent)';
  const tareasColor = checadorColor;
  const academiaColor = checadorColor;
  const nominaColor = checadorColor;
  const isRelojPermitted = true;
  const isTareasPermitted = true;
  const isAcademiaPermitted = true;
  const isNominaPermitted = true;
  const isModuleUnlocked = (_module: string) => true;
