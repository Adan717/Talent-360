...
export function MobileBottomNav({
  ...
  const getModuleColorHex = (modId: string, defaultHex: string) => {
    ...
    return isDark ? tokens.color.brand[100] : tokens.color.functional.accent;
  };

  const checadorColor = getModuleColorHex('asistencia', tokens.color.functional.accent);
  const tareasColor = getModuleColorHex('operativo', tokens.color.functional.accent);
  const academiaColor = getModuleColorHex('academia', tokens.color.functional.accent);
  const nominaColor = getModuleColorHex('facturacion', tokens.color.functional.accent);
    ...
    <div className="relative w-full max-w-[420px] mx-auto select-none font-sans filter drop-shadow-[0_10px_25px_rgba(0,0,0,0.12)]">
      ...
      >
        <path
          ...
          className="fill-white/92 dark:fill-slate-950/92 stroke-navy-100/80 dark:stroke-navy-300/50 backdrop-blur-2xl"
          strokeWidth="1.5"
        />
      </svg>
      ...
      <nav className="relative z-10 pl-5 pr-24 py-1.5 flex items-center justify-around w-full min-h-[64px]">
        ...
        {isRelojPermitted && (
          ...
          >
            <div className={`w-9 h-9 xs:w-10 xs:h-10 rounded-full flex items-center justify-center transition-all ${
              phoneTab === 'checador'
                ? 'bg-accent-soft border-2 border-navy-300 shadow-md shadow-accent/20 scale-105'
        ...
        {isTareasPermitted && (
          ...
          >
            <div className={`w-9 h-9 xs:w-10 xs:h-10 rounded-full flex items-center justify-center transition-all ${
              phoneTab === 'tareas'
                ? 'bg-accent-soft border-2 border-accent shadow-md shadow-accent/20 scale-105'
                : `bg-page/80 dark:bg-slate-900 border border-border/50 dark:border-slate-800/50 hover:bg-slate-200/60 dark:hover:bg-slate-800/60 ${isTareasBlocked || !isModuleUnlocked('operativo') ? 'opacity-40' : ''}`
            }`}>
        ...
        {isAcademiaPermitted && (
          ...
          >
            <div className={`w-9 h-9 xs:w-10 xs:h-10 rounded-full flex items-center justify-center transition-all ${
              phoneTab === 'academia'
                ? 'bg-accent-soft border-2 border-accent dark:border-accent shadow-md shadow-accent/20 scale-105'
                : `bg-page/80 dark:bg-slate-900 border border-border/50 dark:border-slate-800/50 hover:bg-slate-200/60 dark:hover:bg-slate-800/60 ${isAcademiaBlocked || !isModuleUnlocked('academia') ? 'opacity-40' : ''}`
            }`}>
        ...
        {isNominaPermitted && (
          ...
          >
            <div className={`w-9 h-9 xs:w-10 xs:h-10 rounded-full flex items-center justify-center transition-all ${
              phoneTab === 'nomina'
                ? 'bg-accent-soft border-2 border-navy-300 shadow-md shadow-accent/20 scale-105'
                : `bg-page/80 dark:bg-slate-900 border border-border/50 dark:border-slate-800/50 hover:bg-slate-200/60 dark:hover:bg-slate-800/60 ${isNominaBlocked || (!isModuleUnlocked('reportes') && !isModuleUnlocked('facturacion')) ? 'opacity-40' : ''}`
            }`}>
