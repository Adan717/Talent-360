...
export function calculateClockState(ctx: ClockEvaluationContext): ClockStateResult {
  ...
  if (isHoliday) {
    return {
      ...
      bg: 'bg-semantic-info-bg text-semantic-info-text border border-semantic-info-icon',
      icon: '📅',
      iconKey: 'holiday',
      disabled: true,
  ...
  if (isFeatureUnlocked('store_opening') && isOpeningPremium && storeStatus === 'closed') {
    if (Number(userId) === Number(responsibleId)) {
      return {
        ...
        text: 'Abrir Tienda',
        bg: 'bg-functional-accent hover:bg-functional-accent-hover text-white font-black shadow-[0_0_25px_rgba(51,80,126,0.35)] animate-pulse',
        icon: '🗝️',
        iconKey: 'open_store',
  ...
  if (isFeatureUnlocked('store_opening') && Number(userId) === Number(activeEncargadoId) && storeStatus === 'closed') {
    return {
      ...
      stateCode: 'open_store',
      text: 'Abrir Tienda',
      bg: 'bg-functional-accent hover:bg-functional-accent-hover text-white font-bold',
      icon: '🗝️',
      iconKey: 'open_store',
  ...
  if (clockState === 'active') {
    ...
    if (isFeatureUnlocked('enable_ley_silla') && isPro && hasReturnedFromMeal && !hasTakenBreak && features.enable_ley_silla !== false) {
      return {
        ...
        text: 'Tomar Silla',
        bg: 'bg-functional-accent hover:bg-functional-accent-hover text-white font-extrabold shadow-[0_0_20px_rgba(51,80,126,0.3)] animate-pulse',
        icon: '🧘',
        iconKey: 'break_start',
    ...
    if (isFeatureUnlocked('keys_control') && isPro && isManager && !isHandoverCompleted && isHandoverWindow) {
      return {
        ...
        text: 'Entregar Turno',
        bg: 'bg-functional-accent hover:bg-functional-accent-hover text-white font-bold shadow-[0_0_20px_rgba(51,80,126,0.3)] animate-pulse',
        icon: '🗝️',
        iconKey: 'handover',
  ...
  if (clockState === 'short_break') {
    return {
      ...
      bg: 'bg-functional-accent hover:bg-functional-accent-hover text-white font-bold shadow-[0_0_20px_rgba(51,80,126,0.35)]',
      icon: '🏃',
      iconKey: 'break_end',
