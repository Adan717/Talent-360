...
- **Ficha del Colaborador** (clic en la tarjeta) con 4 pestañas y botón **Guardar Ficha**:
  ...
  - **Accesos**: **PIN de kiosco** (seis dígitos con los que la persona se identifica en el kiosco; si es supervisor o administrador, es también el PIN con el que **autoriza una entrada tardía**, valida tareas y abre en emergencia — nunca se muestra, si se olvida se cambia; el sistema rechaza los fáciles como `123456` y los repetidos con otro colaborador); casilla **Colaborador en Activo**; casilla **Permitir acceso web** (al marcarla la persona pasa a Supervisor y aparecen correo de acceso, rol *Supervisor de Confianza* / *Administrador General* y **Nueva contraseña (opcional)**); campos de ID Google/Apple/Samsung; y el bloque **Invitación y Activación Móvil**:
...
- **Entrada tardía: cómo se autoriza (una sola regla).** Pasada la tolerancia, el dial marca *Acceso Bloqueado* y ofrece dos caminos que terminan en el mismo lugar: (a) **el supervisor presente teclea su PIN de kiosco** en la pantalla del colaborador — el servidor lo valida, exige que sea un administrador o supervisor *distinto* al colaborador, y registra la autorización a su nombre; o (b) **"Solicitar autorización"** a distancia: les llega a los administradores y supervisores, y la aprueban en **Monitor 360 → Autorizaciones de entrada**. En ambos casos queda la misma autorización aprobada (con quién la dio) y el siguiente fichaje pasa. No hay QR que escanear: el botón *"[Escaneo Sim.]"* que había era un resto del simulador y generaba el código con la sesión del propio usuario (un administrador se autorizaba solo). El **bloqueo del servidor** se configura en LFT → *Bloqueo por retardo máximo*: en 0 el servidor no bloquea y la única cerradura es la pantalla.
- **Un fichaje rechazado por el servidor ya no se anuncia como registrado.** Si la salida falla (por ejemplo, falta el checklist de cierre), el dial muestra el motivo y la persona sigue en turno; antes decía "Salida registrada" y ofrecía *Tomar Silla* encima del error.
- El **Pase de Lista** se cierra con la ✕ de la esquina.
